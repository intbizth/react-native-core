...
function formatMultilineImport(importCode) {
    ...
    var m = importCode.match(/\{([^}]+)\}/);
    if (m) {
        var arr = _.compact(m[1].split(/, */).map(_.trim));
        if (arr.length) {
...
function addImportFrom(ast, moduleSource, defaultImport, namedImport, namespaceImport) {
    ...
    traverse(ast, {
        ImportDeclaration: function ImportDeclaration(path) {
            ...
            sourceExisted = true;
            var newNames = [];
            var alreadyHaveDefaultImport = !!_.find(node.specifiers, { type: 'ImportDefaultSpecifier' });
            var alreadyHaveNamespaceImport = !!_.find(node.specifiers, { type: 'ImportNamespaceSpecifier' });
            ...
            newNames = newNames.filter(function (n) {
                return !_.find(node.specifiers, function (s) {
                    return s.local.name === n;
                });
            });

            if (newNames.length > 0) {
                var newSpecifiers = [].concat(node.specifiers);
                newNames.forEach(function (n) {
                    var local = t.identifier(n);
                    var imported = local; // TODO: doesn't support local alias.
                    if (n === defaultImport) {
                ...
                var newNode = Object.assign({}, node, { specifiers: newSpecifiers });
                var newCode = generate(newNode, babelGeneratorOptions).code;
    ...
    if (changes.length === 0 && !sourceExisted) {
        ...
        var node = t.importDeclaration(specifiers, t.stringLiteral(moduleSource));
        var code = generate(node, babelGeneratorOptions).code;
        changes.push({
...
function addExportFrom(ast, moduleSource, defaultExport, namedExport) {
    ...
    traverse(ast, {
        ExportNamedDeclaration: function ExportNamedDeclaration(path) {
            ...
            sourceExisted = true;
            var newNames = [];
            var alreadyHaveDefaultExport = !!_.find(node.specifiers, function (s) {
                return _.get(s, 'local.name') === 'default';
            });
            ...
            newNames = newNames.filter(function (n) {
                return !_.find(node.specifiers, function (s) {
                    return (_.get(s, 'exported.name') || _.get(s, 'local.name')) === n;
                });
            });
            ...
            if (newNames.length > 0) {
                ...
                newNames.forEach(function (n) {
                    var local = t.identifier(n);
                    var exported = local; // TODO: doesn't support local alias.
                    if (n === defaultExport) {
    ...
    if (changes.length === 0 && !sourceExisted) {
        ...
        var node = t.ExportNamedDeclaration(null, specifiers, t.stringLiteral(moduleSource));
        var code = generate(node, babelGeneratorOptions).code;
        changes.push({
...
function removeImportSpecifier(ast, name) {
    ...
    traverse(ast, {
        ...
        ImportDeclaration: function ImportDeclaration(path) {
            ...
            var newSpecifiers = node.specifiers.filter(function (s) {
                return !names.includes(s.local.name);
            });
            if (newSpecifiers.length === 0) {
                ...
                var newNode = Object.assign({}, node, { specifiers: newSpecifiers });
                var newCode = generate(newNode, {}).code;
    ...
    traverse(ast, {
        ExportNamedDeclaration: function ExportNamedDeclaration(path) {
            ...
            if (!node.declaration && node.specifiers) {
                var newSpecifiers = _.filter(node.specifiers, function (spec) {
            ...
            // case const
            if (node.declaration) {
                _.forEach(node.declaration.declarations, function (declaration) {
